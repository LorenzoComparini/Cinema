            ...
            <div class="flex flex-col h-full w-full pl-8">
                <div class="flex justify-between w-full items-center">
                    <div>
                        ...
                        <p class="text- italic">{{ reservation.name }} {{ reservation.surname }}</p>
                    </div>


                    <button
                        class="cursor-pointer hover:bg-giallo rounded-full w-12 h-12"
...
export default {
    ...
    methods: {
        async deleteReservation(){
            await axios.delete("http://localhost:8000/api/reservation/delete/:id")
        }
    }
}
</script>